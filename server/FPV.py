...
# Description : Camera processing using OpenCV
...
# Initialize PID controller and set its parameters
pid = PID.PID()
pid.SetKp(0.5)
pid.SetKd(0)
pid.SetKi(0)
...
# Set color thresholds for detection (in HSV)
...
class CVThread(threading.Thread):
    ...
    Y_lock = 0
    X_lock = 0
    tor = 17  # Tolerance value for servo control

    # Initialize servo control and motor functions
    ...
    def mode(self, mode_in, imgInput):
        """Set the current CV mode and input image, then resume processing."""
        self.CVMode = mode_in
        self.imgCV = imgInput
        self.resume()

    def elementDraw(self, imgInput):
        """Draw additional elements on the frame based on current CV mode."""
        if self.CVMode == 'none':
            pass
            ...
            try:
                ...
                cv2.line(imgInput, (self.right_Pos1, (linePos_1+30)), (self.right_Pos1, (linePos_1-30)), (64,128,255), 1)
    ...
    def findLineCtrl(self, posInput, setCenter):
        """Control motor based on line position error."""
    ...
    def findlineCV(self, frame_image):
        """Process frame for line following using computer vision."""
        ...
        # Get line color positions from the image rows at positions linePos_1 and linePos_2
    ...
    def servoMove(ID, Dir, errorInput):
        """Move servo based on error input using Kalman filtering."""
        if ID == 0:
    ...
    def findColor(self, frame_image):
        """Detect color in the frame and control servo accordingly."""
...
class Camera(BaseCamera):
    video_source = -1
    modeSelect = 'none'
    # Uncomment one of the following lines to set a default mode:
    ...
    def frames():
        ...
        while True:
            ret, img = camera.read()
            if img is None:
                print("Error: Unable to read camera data.")
                continue
